...
class Suggest(commands.Cog):
    ...
    async def command(self, ctx, *, suggestion: str):
        ...
        if len(clean_suggestion) < 20:
            await ctx.send(
                "❌ Suggestion is too short. Please provide a detailed suggestion."
            )
            return
    ...
    async def feedback(self, ctx, *, feedback: str):
        ...
        if len(clean_feedback) < 15:
            await ctx.send(
                "❌ Feedback is too short. Please provide a detailed description."
            )
            return
        if len(clean_feedback) > 600:
            await ctx.send(
                "❌ Feedback is too long. Go to <#1363732122866815077> please."
            )
            return

        owner = await self.bot.fetch_user(1085862271399493732)

        await ctx.send("✅ Feedback sent. It will be reviewed as soon as possible.")
        await owner.send(f"Feedback from {ctx.author}:\n> {clean_feedback}")
